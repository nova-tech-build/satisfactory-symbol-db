  ...
  public* blueprints(points: number[]): Generator<Blueprint, void, unknown> {
    ...
    while (rows.length > 0) {
      ...
      if (!blueprint || !keySign) {
        ...
        keySign = keySign
          .withText(`Generated by\nnova-tech-build.github.io/satisfactory-symbol-db`)
          .withAlt(new Date().toISOString())
  ...
  add(name: string, blueprint: Blueprint): void {
    ...
    const summary = Parser.WriteBlueprintFiles(blueprint,
      header => {
        ...
        sbpBodyChunks.push(chunk)
      })

    this.jsZip.file(name + '.sbp', new Uint8Array(Buffer.concat([sbpHeader, ...sbpBodyChunks])))
    this.jsZip.file(name + '.sbpcfg', new Uint8Array(summary.configFileBinary))
  }
}
